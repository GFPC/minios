...
INIT_SRCS := \
	...
	core/process.c core/selinux.c \
	hw/devnodes.c hw/led.c hw/display.c hw/boot_display.c \
	usb/state.c usb/gadget.c usb/adb.c usb/com.c usb/net.c \
	radio/radio.c radio/blockdev.c \
	firmware/fwload.c \
	ui/touch.c

UI_SRCS := ui/kms_paint.c ui/minui.c ui/touch.c ui/ui.c

.PHONY: all init kms_paint wlan_scan ffs_adb propstub drm_dump clean
...
clean:
	rm -f $(INIT_BIN) $(KMS_BIN) $(WLAN_BIN) $(OUT)/ffs_adb $(OUT)/propstub.so $(OUT)/drm_dump
